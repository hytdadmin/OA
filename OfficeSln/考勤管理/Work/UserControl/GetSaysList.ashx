<%@ WebHandler Language="C#" Class="GetSaysList" %>
...
public class GetSaysList : IHttpHandler, IRequiresSessionState
{
    ...
    private string ResultStr(string keyValue, HttpContext context, string userCode, int depId, string searchDate)
    {
        ...
        sayTO.Contents = keyValue.Trim().Equals("输入关键字...") ? "" : keyValue.Trim();
        sayTO.DepId = depId;
        if (searchDate != "undefined" && !string.IsNullOrEmpty(searchDate))
        {
            sayTO.SearchTime = searchDate;
        }
        DataTable dt = new SayBLL().GetSayListByTB(sayTO, pageindex, numPerPage, orderBy, out totalCount);

        sb.AppendFormat("<div class=\"right_content_title\">");
        sb.AppendFormat("全部说说<span id=\"lblCount\">{0}</span>条</div>", totalCount);

        sb.AppendFormat("<table style=\"width:100%\">");
        sb.AppendFormat("<tr>");
        sb.AppendFormat("<td><a id=\"0\" class=\"{0}\" href=\"javascript:InitDataSay(0,{1})\">全部</a>", depId == 0 ? "depB" : "depA", 0);
        List<Department> list = new DepartmentBLL().GetDepListByIsdel();
        if (list != null && list.Count > 0)
        {
            foreach (Department dep in list)
            {
                sb.AppendFormat("丨<a id=\"{1}\" class=\"{2}\" href=\"javascript:InitDataSay(0,{1})\">{0}</a>", dep.RoleName, dep.Id, dep.Id == depId ? "depB" : "depA", dep.Id);
            }
        }
        sb.AppendFormat("</td>");
        sb.AppendFormat("<td></td>");
        sb.AppendFormat("<td style=\"text-align: right;\"><input type=\"text\" value=\"{0}\" id=\"search_date\" readonly=\"readonly\" style=\"border: 0px;width: 80px;text-align: right;\"/></td>", sayTO.SearchTime);
        sb.AppendFormat("<td style=\"width: 6%;\"><div class=\"search_date\"><span  onclick=\"WdatePicker({{el:'search_date',maxDate:'%y-%M-%d',onpicked:function(dp){{InitDataSay(0,{0})}}}})\">时间</span></div></td>", depId);
        sb.AppendFormat("<td style=\"width: 6%;\"><div class=\"search_date\" style=\"width: 60px;\"><span  onclick=\"$('#search_date').val('');InitDataSay(0,{0});\">全部时间</span></div></td>", depId);
        sb.AppendFormat("</tr>");
        ...
        if (dt != null && dt.Rows.Count > 0)
        {
            foreach (DataRow dr in dt.Rows)
            {
                ...
                sb.AppendFormat("</tr>");
                sb.AppendFormat("<tr>");
                sb.AppendFormat("<td class=\"tdcontent\">{0}</td>", PageBase.ShowExpressionHtml(Check.GetString(dr["Contents"]).Replace("\n", "<br/>")));
                sb.AppendFormat("</tr>");
                sb.AppendFormat("<tr>");
